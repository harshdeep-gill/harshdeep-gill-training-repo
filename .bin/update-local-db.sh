...
echo 'Cleaning PII...'
mysql -u root -proot -h 0.0.0.0 quark -e 'UPDATE wp_users SET user_email = CONCAT( "user", wp_users.ID, "@travelopia.com" )' # Reset all users' email address.
mysql -u root -proot -h 0.0.0.0 quark -e 'UPDATE wp_users SET user_pass="$P$BlKV.RDIfk0iBJeyy0m1e5A.kIleIF."' # All users will have the password "password".
