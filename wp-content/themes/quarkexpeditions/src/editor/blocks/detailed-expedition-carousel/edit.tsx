...
export default function Edit( { className, attributes, setAttributes }: BlockEditAttributes ): JSX.Element {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Detailed Expeditions Carousel Options', 'qrk' ) }>
					<SelectControl
					...
					<SelectControl
						label={ __( 'Variation', 'qrk' ) }
						help={ __( 'Select the variation to be displayed', 'qrk' ) }
						value={ attributes.variation }
						options={ [
							{ label: __( 'Home', 'qrk' ), value: 'home' },
							{ label: __( 'Offers', 'qrk' ), value: 'offers' },
						] }
						onChange={ ( variation: string ) => setAttributes( { variation } ) }
					/>
				</PanelBody>
			</InspectorControls>
			<Section className={ classnames( className ) }>
				{
					allOptionsSelected
						? (
							...
							<Placeholder icon="layout" label={ __( 'Detailed Expeditions Carousel', 'qrk' ) }>
								<p>{ __( 'Select one or more options to display Detailed Expeditions Carousel.', 'qrk' ) }</p>
							</Placeholder>
						)
				}
			</Section>
