 ...
 */
const { setState, getState } = zustand.stores.bookDeparturesShips;
...
export const initialize = ( settings: {
	...
	// Get current state.
	const currentState: BookDeparturesShipsState = getState();
	const selectedFilters: BookDeparturesShipsFilters = {
		sort: 'date-now',
	};
...
const filterUpdated = ( response: PartialData ) => {
	...
	const {
		markup,
		data: { resultCount, remainingCount, nextPage },
		noResultsMarkup,
	} = response;
